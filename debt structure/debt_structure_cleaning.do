...
global debt_info = "apq dd1q dlcq dlttq lctq lltq ltq txdbclq xintq dltisy dltry intpny"
...
twoway line txdbclq_mean compustat_dt if QUINTILEdec_BtM==1 & !mi(txdbclq_mean), lw(thin) lc(navy) || line txdbclq_mean compustat_dt if QUINTILEdec_BtM==5 & !mi(txdbclq_mean), lw(thin) lc(dkorange) xlabel(#4, labs(small)) xtitle("Date", size(medsmall)) ytitle("Current deferred tax liability (quarterly, in M$)", size(medsmall)) title("Current Deferred Tax Liability and BtM-Sorted Portfolios (5 vs 1)",size(medlarge)) legend(order(1 "Lowest BtM portfolio" 2 "Highest BtM portfolio")) note("(BtM-sorted quintile portfolios are built following Fama and French (1992))") saving("${figdir}/txdbclq_1.gph", replace)
